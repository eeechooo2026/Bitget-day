...
def get_15m_period_start_timestamp(beijing_dt, offset_periods=0):
    """获取15分钟K线周期的开始时间戳（毫秒，UTC）"""
    total_minutes = beijing_dt.hour * 60 + beijing_dt.minute
    period_minutes = total_minutes // 15 * 15  # 向下取整到15分钟
...
def main():
    ...
    print(f"🚀 开始第22个工作流扫描（15分钟涨幅榜 + 杠杆信息）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 扫描所有USDT本位永续合约")
    print(f"   • 按上根15分钟K棒涨幅从高到低排序")
    ...
    if top:
        ...
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append("💡 解读：上根15分钟K棒涨幅排名，杠杆倍数仅供参考")
