...
def main():
    ...
    print(f"🚀 开始第31个工作流扫描（1小时KDJ金叉 + J值前低 + 按上根振幅×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑（多头）：")
    print(f"   • 1小时级别：上根J值 > 上上根J值（金叉）")
    print(f"   • 1小时级别：上上根J值 < 上上上根J值（前低）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件1（多头）：上根J值 > 上上根J值（金叉）
            if j1 <= j2:
                continue
            # 条件2（多头）：上上根J值 < 上上上根J值（前低）
            if j2 >= j3:
                continue
            ...
            if low1 == 0:
                continue
            amplitude = (high1 - low1) / low1 * 100   # 振幅（正值）
    ...
    msg_lines = [
        f"📊 Bitget 1小时级别KDJ金叉+前低扫描（第31个工作流 - 多头版）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📈 策略逻辑（多头）：",
        f"   • 1小时KDJ金叉：上根J值 > 上上根J值",
        f"   • 1小时J值前低：上上根J值 < 上上上根J值",
        f"   • 排序 = 上根振幅 × (杠杆/100)",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        msg_lines.append(f"📋 金叉+前低 | 振幅×杠杆 前十名（共{len(result_list)}个合约）：")
        ...
        msg_lines.append("💡 解读：1小时KDJ金叉确认 + 振幅×杠杆排序（看涨信号）")
