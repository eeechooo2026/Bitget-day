...
def is_kdj_bearish_arrangement(k, d, j):
    """判断KDJ是否空头排列：J < K < D"""
    return j < k < d
...
def main():
    ...
    print(f"🚀 开始第84个工作流扫描（4小时震荡 + 1小时KDJ空头排列 J<K<D + 上上根非空头 + 排序：50/(杠杆×振幅)从低到高）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 4小时级别：上根收盘 ∈ [上上根区间]（震荡）✅")
    print(f"   • 1小时级别：上根KDJ空头排列（J < K < D）✅")
    print(f"   • 1小时级别：上上根KDJ非空头排列（不满足J < K < D）✅")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件2：上根KDJ空头排列 J<K<D
            if not is_kdj_bearish_arrangement(k1, d1, j1):
                continue

            # 条件3：上上根KDJ非空头排列（不满足J<K<D）
            if is_kdj_bearish_arrangement(k2, d2, j2):
                continue
    ...
    msg_lines = [
        f"📊 Bitget 双周期KDJ空头排列+形态扫描（第84个工作流）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📈 策略逻辑：",
        f"   • 4小时上根震荡 ✅",
        f"   • 1小时上根KDJ空头排列（J<K<D）✅",
        f"   • 1小时上上根KDJ非空头排列 ✅",
        f"   • 排序 = 50 / (杠杆 × 振幅)（从低到高）",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        ...
        msg_lines.append("💡 解读：4小时震荡+1小时KDJ空头排列，排序值越小表示杠杆越高或振幅越大")
