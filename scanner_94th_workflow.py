...
def main():
    ...
    print(f"🚀 开始第94个工作流扫描（4小时级别空头版：上根KDJ空头 + 上上根非空头 + 上根震荡 + 上根收阴 + 按振幅×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 上根KDJ空头排列（J < K < D）✅")
    print(f"   • 上上根KDJ非空头排列（不满足J < K < D）✅")
    print(f"   • 上根收盘价 ∈ [上上根区间]（震荡）✅")
    print(f"   • 上根收阴（收盘价 < 开盘价）✅")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件1（空头）：上根KDJ空头排列（J < K < D）
            if not is_kdj_bearish(k1, d1, j1):
                continue

            # 条件2（空头）：上上根KDJ非空头排列
            if is_kdj_bearish(k2, d2, j2):
                continue

            # 条件3：上根收盘价在上上根区间内（震荡）- 空头也要求震荡
    ...
    msg_lines = [
        f"📉 Bitget 4小时级别KDJ空头+震荡+收阴扫描（第94个工作流 - 空头版）",
    ...
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                ...
                f"   上根: {item['open1']} → {item['close1']} (收阴 ✅) 震荡于 [{item['low2']}, {item['high2']}] ✅"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：KDJ空头+震荡+收阴，按加权振幅排序（看跌信号）")
