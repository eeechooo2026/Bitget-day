...
def main():
    ...
    print(f"🚀 开始第101个工作流扫描（4小时级别：收阴但站上MA5 + 按24h涨幅排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根4小时K棒收阴（收盘价 < 开盘价）✅")
    print(f"   • 上根4小时K棒收盘价 > MA5 ✅")
    print(f"   • 排序 = 24小时涨幅（从高到低）")
    print(f"📊 推送：前十名（微信推送）")
    ...
    try:
        ...
        print(f"📊 共获取 {len(daily_gain_info)} 个合约的24h涨幅")
    except Exception as e:
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ohlcv = exchange.fetch_ohlcv(symbol, timeframe=TIMEFRAME_4H, limit=20)
            if len(ohlcv) < 6:  # 至少需要6根K线
                continue
    ...
    msg_lines = [
        f"📊 Bitget 4小时级别收阴站上MA5扫描（第101个工作流）",
    ...
    if top:
        ...
        msg_lines.append("💡 解读：4小时收阴但站上MA5（回调后仍强势），按24h涨幅排序")
