...
def main():
    ...
    print(f"🚀 开始第54个工作流扫描（1小时：上根MA5>MA20 + 上上根MA5≤MA20 + 按上根涨幅排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根K棒：MA5 > MA20")
    print(f"   • 上上根K棒：MA5 ≤ MA20")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件1：上根 MA5 > MA20
            ma5_1 = calculate_ma_for_target_kline(ohlcv, prev1_ts, MA5_PERIOD)
            ma20_1 = calculate_ma_for_target_kline(ohlcv, prev1_ts, MA20_PERIOD)
            if ma5_1 is None or ma20_1 is None or ma5_1 <= ma20_1:
                continue

            # 条件2：上上根 MA5 ≤ MA20
            ma5_2 = calculate_ma_for_target_kline(ohlcv, prev2_ts, MA5_PERIOD)
            ma20_2 = calculate_ma_for_target_kline(ohlcv, prev2_ts, MA20_PERIOD)
            if ma5_2 is None or ma20_2 is None or ma5_2 > ma20_2:
                continue
    ...
    msg_lines = [
        f"📊 Bitget 1小时级别MA5/MA20形态扫描（第54个工作流）",
    ...
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                f"{i}. {item['symbol']}\n"
                f"   上根涨幅: +{item['gain']}%\n"
                f"   上根: MA5={item['ma5_1']} > MA20={item['ma20_1']} ✅\n"
                f"   上上根: MA5={item['ma5_2']} ≤ MA20={item['ma20_2']} ✅\n"
                f"   杠杆: {item['leverage']}x"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：上根MA5上穿MA20，上上根MA5在MA20下方，按上根涨幅排序")
