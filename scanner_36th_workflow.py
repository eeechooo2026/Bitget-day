...
def main():
    ...
    print(f"🚀 开始第36个工作流扫描（1小时双K线震荡形态 + 按振幅和×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根K棒收盘价在上上根K棒的最高价和最低价之间（震荡）")
    print(f"   • 上上根K棒收盘价在上上上根K棒的最高价和最低价之间（震荡）")
    print(f"   • 排序指标 = (上根振幅 + 上上根振幅) × (最高杠杆倍数 / 100)")
    ...
    msg_lines = [
        f"📊 Bitget 1小时级别双K线震荡扫描（第36个工作流）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📈 策略逻辑：",
        f"   • 上根收盘价 ∈ [上上根最低价, 上上根最高价]",
        f"   • 上上根收盘价 ∈ [上上上根最低价, 上上上根最高价]",
        f"   • 排序 = (上根振幅 + 上上根振幅) × (杠杆/100)",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        msg_lines.append(f"📋 双K线震荡 | 振幅和×杠杆 前十名（共{len(result_list)}个合约）：")
        for i, item in enumerate(top, 1):
            msg_lines.append(
                f"{i}. {item['symbol']}\n"
                f"   上上上根区间: [{item['low3']}, {item['high3']}], 上上根收盘: {item['close2']} ✅\n"
        ...
        msg_lines.append("💡 解读：连续两根K线在各自前一根K线区间内震荡，按波动强度排序")
