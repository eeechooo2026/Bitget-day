...
def main():
    ...
    print(f"🚀 开始第33个工作流扫描（1小时均线空头 + 上根收阳 + 上上根收阴 + 按上上根振幅×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 1小时级别：均线空头排列（MA5 < MA10 < MA20）")
    print(f"   • 上根K棒收阳（收盘价 > 开盘价）")
    ...
    msg_lines = [
        f"📉 Bitget 1小时级别均线空头+阴阳形态扫描（第33个工作流 - 空头版）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📉 策略逻辑（空头）：",
        f"   • 均线空头排列：MA5 < MA10 < MA20",
        f"   • 上根收阳（收盘 > 开盘）",
        f"   • 上上根收阴（收盘 < 开盘）",
        f"   • 排序 = 上上根振幅 × (杠杆/100)",
        f"━━━━━━━━━━━━━━━━━━━━"
    ...
    if top:
        ...
        msg_lines.append("💡 解读：均线空头排列，上根反弹收阳，上上根阴线，按上上根波动强度排序（看跌信号）")
