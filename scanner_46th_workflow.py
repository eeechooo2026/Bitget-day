...
def main():
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if open1 == 0 or open2 == 0 or low2 == 0:
                continue
    ...
    # 按上上根涨幅从高到低排序
    result_list.sort(key=lambda x: x['gain2'], reverse=True)
    top = result_list[:PUSH_TOP_N]

    current_time = beijing_now.strftime('%Y-%m-%d %H:%M')
    msg_lines = [
        f"📊 Bitget 4小时级别（上根收阳+震荡 + 上上根收阳）扫描（第46个工作流）",
    ...
    if top:
        ...
        msg_lines.append("💡 解读：上根在上上根区间内震荡收阳，上上根也收阳，按上上根涨幅排序")
