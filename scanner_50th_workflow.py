...
def main():
    ...
    print(f"🚀 开始第50个工作流扫描（4小时：空头版多重形态组合 + 双K线震荡 + 按振幅和×杠杆/100排序）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if low1 == 0 or low2 == 0 or low3 == 0 or low4 == 0:
                continue
            ...
            # 条件3（空头）：上上上根收阴 + 收盘价 < 上上上上根最低价（跌破前低）
            if close3 >= open3:
                continue
            if close3 >= low4:
                continue
    ...
    msg_lines = [
        f"📉 Bitget 4小时级别多重形态扫描（第50个工作流 - 空头版）",
    ...
    if top:
        ...
        msg_lines.append("💡 解读：空头版多重形态组合（跌破前低+双震荡），按双K线加权波动强度排序")
