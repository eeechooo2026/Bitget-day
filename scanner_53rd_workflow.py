...
def main():
    ...
    print(f"🚀 开始第53个工作流扫描（1小时：上根均线多头 + 上上根非多头 + 按上根涨幅排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根收阳 + 均线多头排列（MA5 > MA10 > MA20）")
    print(f"   • 上上根K棒不符合均线多头排列")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件1续：上根均线多头排列
            ma5_1 = calculate_ma_for_target_kline(ohlcv, prev1_ts, 5)
            ma10_1 = calculate_ma_for_target_kline(ohlcv, prev1_ts, 10)
            ma20_1 = calculate_ma_for_target_kline(ohlcv, prev1_ts, 20)
            if not is_bullish_arrangement(ma5_1, ma10_1, ma20_1):
                continue

            # 条件2：上上根不符合均线多头排列
            ma5_2 = calculate_ma_for_target_kline(ohlcv, prev2_ts, 5)
            ma10_2 = calculate_ma_for_target_kline(ohlcv, prev2_ts, 10)
            ma20_2 = calculate_ma_for_target_kline(ohlcv, prev2_ts, 20)
            if is_bullish_arrangement(ma5_2, ma10_2, ma20_2):
                continue
            ...
            result_list.append({
                ...
                'open1': round(open1, 4),
                'ma5_1': round(ma5_1, 4),
                'ma10_1': round(ma10_1, 4),
                'ma20_1': round(ma20_1, 4),
            })
    ...
    msg_lines = [
        f"📊 Bitget 1小时级别均线形态扫描（第53个工作流）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📈 策略逻辑：",
        f"   • 上根收阳 + 均线多头排列（MA5 > MA10 > MA20）",
        f"   • 上上根不符合均线多头排列",
        f"   • 排序 = 上根涨幅",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                f"{i}. {item['symbol']}\n"
                f"   上根涨幅: +{item['gain']}%\n"
                f"   上根均线: {item['ma5_1']} > {item['ma10_1']} > {item['ma20_1']} ✅\n"
                f"   上根: {item['open1']} → {item['close1']} (收阳 ✅)\n"
                f"   杠杆: {item['leverage']}x"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：上根均线多头+收阳，上上根非多头，按上根涨幅排序")
