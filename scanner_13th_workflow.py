...
# ========== TradFi 外汇交易对列表（手动维护）==========
# 格式：交易所实际使用的symbol名称
# 根据Bitget公告，外汇CFD交易对的symbol格式通常为 EURCZK, USDCZK 等
# 请在Bitget行情页确认实际symbol后修改此列表
TRADFI_FOREX_SYMBOLS = [
    "EURCZK/USDT:USDT",   # 欧元/捷克克朗
    "EURDKK/USDT:USDT",   # 欧元/丹麦克朗
    "USDCZK/USDT:USDT",   # 美元/捷克克朗
    "USDDKK/USDT:USDT",   # 美元/丹麦克朗
    "USDIDR/USDT:USDT",   # 美元/印尼盾
    "USDINR/USDT:USDT",   # 美元/印度卢比
    "USDTWD/USDT:USDT",   # 美元/新台币
    # 以下为基础外汇对（如确认上线，请取消注释并验证symbol格式）
    # "EURUSD/USDT:USDT",   # 欧元/美元
    # "GBPUSD/USDT:USDT",   # 英镑/美元
    # "USDJPY/USDT:USDT",   # 美元/日元
    # "AUDUSD/USDT:USDT",   # 澳元/美元
    # "USDCAD/USDT:USDT",   # 美元/加元
    # "USDCHF/USDT:USDT",   # 美元/瑞郎
    # "NZDUSD/USDT:USDT",   # 纽元/美元
    # "EURGBP/USDT:USDT",   # 欧元/英镑
    # "EURJPY/USDT:USDT",   # 欧元/日元
    # "GBPJPY/USDT:USDT",   # 英镑/日元
]
# =============================================
...
def main():
    ...
    print(f"🚀 开始第13个工作流扫描（Bitget TradFi 外汇产品） - 当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    ...
    print(f"📋 扫描范围：手动指定的TradFi外汇交易对，共{len(TRADFI_FOREX_SYMBOLS)}个")

    exchange = ccxt.bitget({'enableRateLimit': True, 'options': {'defaultType': 'swap'}})

    scan_symbols = TRADFI_FOREX_SYMBOLS
    ...
    for idx, symbol in enumerate(scan_symbols):
        try:
            ...
            if not is_bullish_arrangement(ma5, ma10, ma20):
                print(f"      ❌ {symbol} 均线非多头")
                continue
            if close1 <= ma5:
                print(f"      ❌ {symbol} 收盘价 {close1:.4f} <= MA5 {ma5:.4f}")
                continue

            # 条件2：上上根震荡
            if not is_consolidation_kline(close2, high3, low3):
                print(f"      ❌ {symbol} 上上根未震荡")
                continue

            # 条件3：上根震荡
            if not is_consolidation_kline(close1, high2, low2):
                print(f"      ❌ {symbol} 上根未震荡")
                continue
    ...
    msg_lines = [
        f"📊 Bitget TradFi 外汇4小时级别扫描（第13个工作流）",
    ...
    if top:
        msg_lines.append(f"📋 推送前十名（共{len(result_list)}个符合条件的TradFi外汇产品）：")
        ...
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的TradFi外汇产品")
        msg_lines.append("💡 解读：均线多头+双K线区间震荡+J值上升，日线级别上涨确认")
        msg_lines.append("⚠️ 此信息仅供参考，不构成投资建议")
    else:
        msg_lines.append("😔 今日未找到符合条件的TradFi外汇产品")
