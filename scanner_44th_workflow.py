...
def main():
    ...
    print(f"🚀 开始第44个工作流扫描（1小时：收盘>MA5 + J值上升 + 双K线震荡）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根收盘价 > MA5")
    print(f"   • 上根KDJ的J值 > 上上根J值")
    print(f"   • 上上根收盘价 ∈ [上上上根最低价, 上上上根最高价]")
    print(f"   • 上上上根收盘价 ∈ [上上上上根最低价, 上上上上根最高价]")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if open1 == 0 or low1 == 0 or low3 == 0 or low4 == 0:
                continue
            ...
            # 条件3：上上根收盘价在上上上根的最高价和最低价之间
            if not (low3 < close2 < high3):
                continue

            # 条件4：上上上根收盘价在上上上上根的最高价和最低价之间
    ...
    if top:
        ...
        msg_lines.append("💡 解读：收盘站上MA5 + J值上升 + 双K线震荡，按加权涨幅排序")
