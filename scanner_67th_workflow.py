...
def main():
    ...
    print(f"🚀 开始第67个工作流扫描（日线级别：阳-阴(震荡)-阳形态 + 按涨幅从高到低排序）")
    ...
    msg_lines = [
        f"📊 Bitget 日线级别阳-阴(震荡)-阳形态扫描（第67个工作流）",
    ...
    if top:
        msg_lines.append(f"📋 涨幅最大前十名（共{len(result_list)}个合约）：")
        ...
        msg_lines.append("💡 解读：日线形成阳-阴(震荡)-阳形态，按最新日线涨幅从大到小排序")
