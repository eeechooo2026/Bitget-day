...
def main():
    ...
    print(f"🚀 开始第十个工作流扫描（4小时级别空头） - 北京时间 {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 上根4小时K棒均线空头排列（MA5 < MA10 < MA20，且上根K棒收盘价 < MA5）")
    print(f"   • 上根4小时K棒收阴、最高价高于上上根K棒的最高价、且KDJ的J值小于上上根")
    ...
    msg_lines = [
        f"📊 Bitget 4小时级别空头扫描（第十个工作流）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📉 策略逻辑（空头）：",
        f"   • 上根4小时K棒均线空头排列（MA5<MA10<MA20，且收盘价<MA5）",
        f"   • 上根4小时K棒收阴、最高价高于上上根最高价、KDJ的J值小于上上根",
        f"📊 排序：按前两根日线K棒累计跌幅从高到低（跌幅越大越靠前）",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    ...
    if top_results:
        ...
        msg_lines.append("💡 解读：4小时级别均线空头+创新高+J值下降，日线级别正处下跌趋势")
