...
def main():
    ...
    print(f"🚀 开始第92个工作流扫描（1小时级别：上根KDJ多头 + J值上升 + 上上根J值前低 + 震荡 + 收阳 + 按振幅×杠杆/100排序）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            j3 = j_vals[idx3]
            if None in (k1, d1, j1, k2, d2, j2, j3):
                continue

            # 条件1a：上根KDJ多头排列（J > K > D）
            if not is_kdj_bullish(k1, d1, j1):
                continue

            # 条件1b：上根J值 > 上上根J值（J值上升）
            if j1 <= j2:
                continue

            # 条件2：上上根J值 < 上上上根J值（J值前低）
            if j2 >= j3:
                continue
    ...
    msg_lines = [
        f"📊 Bitget 1小时级别KDJ多头+J值上升+前低扫描（第92个工作流）",
    ...
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                ...
                f"   J值变化: {item['j3']} → {item['j2']} → {item['j1']} (J2<J3 ✅, J1>J2 ✅)\n"
                f"   上根: {item['open1']} → {item['close1']} (收阳 ✅) 震荡于 [{item['low2']}, {item['high2']}] ✅"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：KDJ金叉+J值上升+前低确认+震荡收阳，按加权振幅排序")
