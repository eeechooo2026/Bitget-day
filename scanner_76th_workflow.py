...
def main():
    ...
    print(f"🚀 开始第76个工作流扫描（4小时级别：上根震荡 + 上上根不震荡 + 按|涨跌幅|×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根震荡：收盘价 ∈ [上上根区间]")
    print(f"   • 上上根不震荡：收盘价 ∉ [上上上根区间]")
    print(f"   • 排序 = |上根涨跌幅| × (最高杠杆倍数 / 100)（从高到低）")
    print(f"   • 涨跌幅 = (收盘价 - 开盘价) / 开盘价 × 100%")
    print(f"📊 推送：前十名（微信推送）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if open1 == 0 or low2 == 0 or low3 == 0:
                continue
    ...
    msg_lines = [
        f"📊 Bitget 4小时级别震荡形态扫描（第76个工作流）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📈 策略逻辑：",
        f"   • 上根震荡：收盘 ∈ [上上根区间] ✅",
        f"   • 上上根不震荡：收盘 ∉ [上上上根区间] ✅",
        f"   • 排序 = |上根涨跌幅| × (杠杆/100)（从高到低）",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        ...
        msg_lines.append("💡 解读：上根震荡，上上根不震荡，按加权波动幅度排序（不分涨跌）")
