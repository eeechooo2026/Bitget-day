...
def main():
    ...
    print(f"🚀 开始第88个工作流扫描（4小时级别空头版：上根收阴震荡 + 上上根收阳震荡 + 上上上根跌破前低 + 按|上上上根跌幅|×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 上根收阴 + 收盘 ∈ [上上根区间]（震荡）✅")
    print(f"   • 上上根收阳 + 收盘 ∈ [上上上根区间]（震荡）✅")
    print(f"   • 上上上根收阴 + 收盘 < 上上上上根最低价（跌破前低）✅")
    print(f"   • 排序 = |上上上根跌幅| × (最高杠杆倍数 / 100)（从高到低）")
    print(f"📊 推送：前十名（微信推送）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if any(v == 0 for v in [open1, open2, open3, low2, low3, low4]):
                continue

            # 条件1（空头）：上根收阴 + 收盘价在上上根区间内
            if close1 >= open1:
                continue
            if not (low2 < close1 < high2):
                continue

            # 条件2（空头）：上上根收阳 + 收盘价在上上上根区间内
            if close2 <= open2:
                continue
            if not (low3 < close2 < high3):
                continue

            # 条件3（空头）：上上上根收阴 + 收盘价 < 上上上上根最低价
            if close3 >= open3:
                continue
            if close3 >= low4:
                continue
            ...
            # 排序指标：|上上上根跌幅| × 杠杆/100
            change3 = (close3 - open3) / open3 * 100  # 负值表示下跌
            abs_change3 = abs(change3)
            leverage = leverage_info[symbol]
            score = abs_change3 * (leverage / 100)

            result_list.append({
                'symbol': symbol.replace('/USDT:USDT', ''),
                'change3': round(change3, 2),
                'abs_change3': round(abs_change3, 2),
    ...
    msg_lines = [
        f"📉 Bitget 4小时级别多重形态扫描（第88个工作流 - 空头版）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📉 策略逻辑（空头）：",
        f"   • 上根收阴 + 收盘 ∈ [上上根区间] ✅",
        f"   • 上上根收阳 + 收盘 ∈ [上上上根区间] ✅",
        f"   • 上上上根收阴 + 收盘 < 上上上上根最低价 ✅",
        f"   • 排序 = |上上上根跌幅| × (杠杆/100)",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
