...
def main():
    ...
    print(f"🚀 开始第60个工作流扫描（4小时空头版：收盘<MA20 + 跌破前{LOOKBACK_BARS}根最低价 + 按跌幅×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 上根K棒收盘价 < MA20")
    print(f"   • 上根K棒收盘价 < 前{LOOKBACK_BARS}根K棒每一根的最低价（跌破前低）")
    print(f"   • 排序 = |上根跌幅| × (杠杆/100)（从高到低）")
    print(f"📊 推送：前十名（微信推送）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件2（空头）：上根收盘价 < 前 LOOKBACK_BARS 根K棒的最低价
            idx1 = None
            for i, k in enumerate(ohlcv):
                if k[0] == prev1_ts:
                    idx1 = i
            ...
            # 排序指标：|上根跌幅| × 杠杆/100
            change = (close1 - open1) / open1 * 100  # 负值表示下跌
    ...
    # 按 score 从高到低排序（跌幅越大，score 越大，排越前）
    result_list.sort(key=lambda x: x['score'], reverse=True)
    top = result_list[:PUSH_TOP_N]

    current_time = beijing_now.strftime('%Y-%m-%d %H:%M')
    msg_lines = [
        f"📉 Bitget 4小时级别空头突破扫描（第60个工作流）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📉 策略逻辑（空头）：",
        f"   • 上根收盘价 < MA20",
        f"   • 上根收盘价 < 前{LOOKBACK_BARS}根最低价",
        f"   • 排序 = |上根跌幅| × (杠杆/100)",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                ...
                f"   跌破前{LOOKBACK_BARS}根最低: {item['min_low']} → {item['close1']} ✅\n"
                f"   杠杆: {item['leverage']}x\n"
                f"   排序值: {item['score']}"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append(f"💡 解读：收盘跌破MA20且跌破前{LOOKBACK_BARS}根低点，按加权跌幅排序")
