...
def main():
    ...
    print(f"🚀 开始第99个工作流扫描（1小时级别：上根收阴震荡 + 上上根突破 + 按上上根涨幅×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根收阴（收盘 < 开盘）✅")
    print(f"   • 上根收盘 ∈ [上上根区间]（震荡）✅")
    print(f"   • 上上根收阳（收盘 > 开盘）✅")
    print(f"   • 上上根收盘 > 上上上根最高价（突破前高）✅")
    print(f"   • 上根最低价 ≤ 上上上根最高价 ✅")
    print(f"   • 排序 = 上上根涨幅 × (杠杆/100)（从高到低）")
    print(f"📊 推送：前十名（微信推送）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if any(v == 0 for v in [open1, open2, low2, high3]):
                continue
    ...
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                ...
                f"   排序值: {item['score']}\n"
                f"   上上上根最高: {item['high3']}\n"
                f"   上上根: {item['open2']} → {item['close2']} (收阳 ✅) 突破前高 ✅\n"
                f"   上根: {item['open1']} → {item['close1']} (收阴 ✅) 最低 {item['low1']} ≤ {item['high3']} ✅, 收盘 ∈ [{item['low2']}, {item['high2']}] ✅"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：上上根突破前高，上根回调收阴震荡且未破前高，按加权涨幅排序")
