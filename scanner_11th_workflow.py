...
def is_bearish_arrangement(ma5, ma10, ma20):
...
def calculate_daily_loss(ohlcv_1d, target_ts, prev_ts):
    ...
    if close_prev == 0:
        return None
    return (close_target - close_prev) / close_prev * 100  # 负值表示下跌
...
def main():
    ...
    print(f"🚀 开始第五个工作流空头版扫描 - 当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 上根4小时K棒均线空头排列（MA5 < MA10 < MA20，且收盘价 < MA5）")
    print(f"   • 上根和上上根4小时K棒均处于震荡（收盘价落于前一根区间内）")
    print(f"   • 上根KDJ的J值 < 上上根KDJ的J值")
    print(f"📊 排序：按前两根日线K棒跌幅从高到低")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件1：均线空头排列（基于上根计算MA5, MA10, MA20）
            ...
            if not is_bearish_arrangement(ma5, ma10, ma20):
                continue
            if close1 >= ma5:   # 空头要求收盘价 < MA5
                continue
    ...
    # 按日线跌幅从高到低排序（跌幅越大即 loss_1d 越小，升序排列）
    result_list.sort(key=lambda x: x['loss_1d'])
    top = result_list[:PUSH_TOP_N]

    current_time = beijing_now.strftime('%Y-%m-%d %H:%M')
    msg_lines = [
        f"📉 Bitget 4小时级别空头扫描（第五个工作流空头版）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📉 策略逻辑：",
        f"   • 上根均线空头排列（MA5<MA10<MA20，且收盘<MA5）",
        f"   • 上上根震荡（收盘介于上上上根区间）",
        f"   • 上根震荡（收盘介于上上根区间）",
        f"   • 上根J值 < 上上根J值",
        f"📊 排序：按前两根日线跌幅从高到低",
    ...
    if top:
        ...
        msg_lines.append("💡 解读：均线空头+双K线区间震荡+J值下降，日线级别下跌确认")
