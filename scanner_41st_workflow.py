...
def main():
    ...
    print(f"🚀 开始第41个工作流扫描（日线级别空头版：上根收阴 + 收盘价 > MA20 + 按上根振幅排序）")
    ...
    msg_lines = [
        f"📉 Bitget 日线级别空头版（上根收阴 + 收盘>MA20）扫描（第41个工作流）",
    ...
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                ...
                f"   高低: [{item['low1']}, {item['high1']}]"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：日线收阴但仍在MA20上方，按波动强度排序（看跌信号）")
