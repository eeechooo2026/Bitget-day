...
def main():
    ...
    print(f"🚀 开始第51个工作流扫描（1小时：四重形态组合 + 按4小时上根振幅×杠杆/100排序）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            # 条件2：上上根收阴 + 收盘在上上上根区间内
            if close2 >= open2:
                continue
            if not (low3 < close2 < high3):
                continue

            # 条件3：上上上根收阴 + 收盘在上上上上根区间内
            if close3 >= open3:
                continue
            if not (low4 < close3 < high4):
                continue

            # 条件4：上上上上根收阳 + 收盘 > 上上上上上根最高价
            if close4 <= open4:
                continue
            if close4 <= high5:
                continue
            ...
            if low_4h == 0:
                continue
            amplitude_4h = (high_4h - low_4h) / low_4h * 100
            leverage = leverage_info[symbol]
            score = amplitude_4h * (leverage / 100)

            result_list.append({
                'symbol': symbol.replace('/USDT:USDT', ''),
                'amplitude_4h': round(amplitude_4h, 2),
    ...
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                f"{i}. {item['symbol']}\n"
                f"   上上上上上根最高: {item['high5']}\n"
                f"   上上上上根: {item['open4']} → {item['close4']} (收阳 ✅) 突破前高 ✅\n"
                ...
                f"   4小时上根振幅: {item['amplitude_4h']}%, 杠杆: {item['leverage']}x\n"
                f"   排序值: {item['score']}"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：四重形态组合（突破前高+三重震荡），按4小时加权波动强度排序")
