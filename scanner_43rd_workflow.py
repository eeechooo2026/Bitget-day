...
def main():
    ...
    print(f"🚀 开始第43个工作流扫描（1小时跌幅×杠杆/100排序 - 空头版）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 扫描所有USDT本位永续合约")
    print(f"   • 排序指标 = 上根1小时K棒跌幅 × (最高杠杆倍数 / 100)")
    print(f"   • 跌幅为负值，按指标值从低到高排序（跌幅最大的排最前）")
    print(f"   • 附：每个币种的最高合约杠杆倍数")
    print(f"📊 推送：前十名（微信推送）")
    ...
    msg_lines = [
        f"📉 Bitget 1小时级别跌幅×杠杆/100排行（第43个工作流 - 空头版）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📉 策略逻辑（空头）：",
        f"   • 扫描所有USDT本位永续合约",
        f"   • 排序指标 = 上根1小时K棒跌幅 × (杠杆/100)",
        f"   • 跌幅为负值，按指标值从低到高排序（跌幅最大的排最前）",
        f"   • 附：每个币种的最高合约杠杆倍数",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        msg_lines.append(f"📋 跌幅×杠杆 排行榜前十名（共{len(result_list)}个合约）：")
