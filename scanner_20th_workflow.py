...
def main():
    ...
    print(f"🚀 开始第20个工作流扫描（1小时涨幅×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 扫描所有USDT本位永续合约")
    print(f"   • 排序指标 = 上根1小时K棒涨幅 × (最高杠杆倍数 / 100)")
    ...
    msg_lines = [
        f"📊 Bitget 1小时级别涨幅×杠杆/100排行（第20个工作流）",
        f"🕘 时间：{current_time}（北京时间）",
        f"📈 策略逻辑：",
        f"   • 扫描所有USDT本位永续合约",
        f"   • 排序指标 = 上根1小时K棒涨幅 × (杠杆/100)",
        f"   • 附：每个币种的最高合约杠杆倍数",
        f"━━━━━━━━━━━━━━━━━━━━"
    ]
    if top:
        msg_lines.append(f"📋 涨幅×杠杆 排行榜前十名（共{len(result_list)}个合约）：")
