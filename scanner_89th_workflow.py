...
def main():
    ...
    print(f"🚀 开始第89个工作流扫描（4小时级别：双震荡 + 内包形态 + 按上根振幅×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📈 策略逻辑：")
    print(f"   • 上根收盘价 ∈ [上上根区间]（震荡）✅")
    print(f"   • 上根最高价 > 上上根最高价（上破前高）✅")
    print(f"   • 上根最低价 < 上上根最低价（下破前低）✅")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if any(v == 0 for v in [low1, low2, low3]):
                continue
            ...
            # 条件2：上根最高价 > 上上根最高价（上破前高）
            if high1 <= high2:
                continue

            # 条件3：上根最低价 < 上上根最低价（下破前低）
            if low1 >= low2:
                continue
    ...
    if top:
        ...
        for i, item in enumerate(top, 1):
            msg_lines.append(
                ...
                f"   上上根: 收盘 {item['close2']} ∈ 区间 ✅, 高低 [{item['low2']}, {item['high2']}]\n"
                f"   上根: 收盘 {item['close1']} ∈ [{item['low2']}, {item['high2']}] ✅\n"
                f"   内包: 最高 {item['high1']} > {item['high2']} ✅, 最低 {item['low1']} < {item['low2']} ✅"
            )
        msg_lines.append("━━━━━━━━━━━━━━━━━━━━")
        msg_lines.append(f"📊 共筛选出 {len(result_list)} 个符合条件的币种")
        msg_lines.append("💡 解读：双震荡 + 内包形态（高低点同时突破），按加权波动强度排序")
