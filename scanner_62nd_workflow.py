...
def main():
    ...
    print(f"🚀 开始第62个工作流扫描（1小时空头版：收阳 + 收盘<MA5 + 按|4小时跌幅|×杠杆/100排序）")
    print(f"   当前北京时间: {beijing_now.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📉 策略逻辑（空头）：")
    print(f"   • 上根1小时K棒收阳（收盘价 > 开盘价）")
    print(f"   • 上根1小时K棒收盘价 < MA5")
    print(f"   • 排序 = |4小时上根跌幅| × (最高杠杆倍数 / 100)")
    print(f"📊 推送：前十名（微信推送）")
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            if open_4h == 0:
                continue
            change_4h = (close_4h - open_4h) / open_4h * 100  # 负值表示下跌
    ...
    msg_lines = [
        f"📉 Bitget 1小时级别收阳+收盘<MA5扫描（第62个工作流 - 空头版）",
    ...
    if top:
        ...
        msg_lines.append("💡 解读：1小时收阳但跌破MA5，按4小时加权跌幅排序（看跌信号）")
