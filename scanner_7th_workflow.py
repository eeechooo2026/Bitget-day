...
def get_period_start_timestamp(beijing_dt, offset_periods=0, timeframe_minutes=240):
    """
...
def calculate_daily_gain(ohlcv_1d, target_ts, prev_ts):
    """
    计算指定日线K线相对于前一根日线K线的涨幅
    target_ts: 前一根日线K线（较新）的时间戳
    prev_ts:   前前一根日线K线（较旧）的时间戳
    """
...
def main():
    ...
    # 日线级别：用于排序，取最近两根已收盘的日线
    # 注意：当前日期可能未收盘，所以取前两个完整的日线
    # 例如当前是4月19日，则取4月18日和4月17日的日线
    today_start = beijing_now.replace(hour=0, minute=0, second=0, microsecond=0)
    prev1_ts_1d = get_period_start_timestamp(today_start, -1, 1440)   # 前一天日线（UTC 00:00）
    prev2_ts_1d = get_period_start_timestamp(today_start, -2, 1440)   # 前两天日线
    ...
    for idx, symbol in enumerate(swap_symbols):
        try:
            ...
            ma_values_4h = calculate_moving_averages(closes_4h, MA_PERIODS)
            
            # 条件1：均线多头排列
            if not is_bullish_arrangement(ma_values_4h):
                continue
            
            # 精确查找目标K线
            k_prev1_4h = find_kline_by_timestamp(ohlcv_4h, prev1_ts_4h)  # 上根
            k_prev2_4h = find_kline_by_timestamp(ohlcv_4h, prev2_ts_4h)  # 上上根
            
            if not (k_prev1_4h and k_prev2_4h):
                continue
            
            # 提取K线数据
            close1 = k_prev1_4h[4]   # 上根收盘价
            open1 = k_prev1_4h[1]    # 上根开盘价
            low1 = k_prev1_4h[3]     # 上根最低价
            low2 = k_prev2_4h[3]     # 上上根最低价
            
            # 条件2：上根K棒价格 > MA5
            if close1 <= ma_values_4h[5]:
                continue
            ...
            _, _, j_vals_4h = calculate_kdj(highs_4h, lows_4h, closes_4h, KDJ_RSV_PERIOD, KDJ_SMOOTH)
            ...
            if idx1 is None or idx2 is None or j_vals_4h[idx1] is None or j_vals_4h[idx2] is None:
                continue
            
            j_prev1 = j_vals_4h[idx1]
            j_prev2 = j_vals_4h[idx2]
            ...
            result_list.append({
                ...
                'ma5': round(ma_values_4h[5], 4),
                'ma10': round(ma_values_4h[10], 4),
                'ma20': round(ma_values_4h[20], 4),
            ...
            print(f"✓ {symbol} 满足4小时条件，日线涨幅{gain_1d:.2f}%")
