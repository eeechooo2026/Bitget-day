...
def main():
    ...
    # 按上上上根涨幅从高到低排序
    result_list.sort(key=lambda x: x['gain3'], reverse=True)
    top = result_list[:PUSH_TOP_N]

    current_time = beijing_now.strftime('%Y-%m-%d %H:%M')
    msg_lines = [
        f"📊 Bitget 4小时级别多重形态扫描（第98个工作流）",
    ...
    if top:
        ...
        msg_lines.append("💡 解读：双震荡 + 突破前高，按上上上根涨幅排序")
